...
import { bulldozer, scira } from '@/ai/providers';
import {
  getChatsByUserId,
  deleteChatById,
...
export async function suggestQuestions(history: any[]) {
  ...
  const { object } = await generateObject({
    model: bulldozer.languageModel('bulldozer-default'),
    temperature: 0,
    maxOutputTokens: 512,
    system: `You are Local 825's Bulldozer Brain AI research assistant. Generate EXACTLY 3 follow-up questions for labor organizing research based on the conversation history.

### Labor Organizing Question Guidelines:
- Create exactly 3 questions focused on labor organizing, union strategy, and worker empowerment
- Questions must be concise (5-12 words) but specific to organizing contexts
- Prioritize company research, labor law, contract analysis, and organizing tactics
- NEVER use pronouns - always use specific company names, union terms, or clear context
- Questions should advance organizing intelligence and strategic research
- Focus on actionable research that supports worker organizing efforts

### Labor-Specific Question Types:
- Company Research: Corporate structure, ownership, financials, labor violations
- Contract Analysis: Collective bargaining agreements, wage scales, benefits
- Labor Law: NLRB cases, labor violations, regulatory compliance
- Organizing Strategy: Union campaigns, worker rights, organizing tactics
- Public Documents: SEC filings, court cases, government contracts
- Industry Analysis: Sector trends, competitor analysis, market conditions
- Worker Safety: OSHA violations, workplace incidents, safety records
- Corporate Accountability: Executive compensation, political donations, lobbying

### Context Transformation for Labor Focus:
- For general company mentions → Generate questions about labor practices, union history
- For financial discussions → Focus on worker compensation, benefit cuts, executive pay ratios
- For legal topics → Explore labor law implications, worker rights, NLRB cases
- For industry news → Analyze impact on workers, organizing opportunities
- For location-based queries → Research local labor markets, union presence, organizing history
...
- Questions must advance labor organizing intelligence
- Focus on research that empowers workers and supports organizing`,
    messages: history,
    schema: z.object({
