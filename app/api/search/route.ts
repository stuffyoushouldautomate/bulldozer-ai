...
import { bulldozer, scira, requiresAuthentication, requiresProSubscription, shouldBypassRateLimits, models } from '@/ai/providers';
import {
  createStreamId,
  getChatById,
  saveChat,
  ...
  const stream = createUIMessageStream({
    execute: async ({ writer: dataStream }) => {
      ...
      const result = streamText({
        model: bulldozer.languageModel(model),
