...
export default function PricingTable({ subscriptionDetails, user }: PricingTableProps) {
  ...
  // If payment environment variables are missing, payments are disabled
  const paymentsEnabled = !!(STARTER_TIER && STARTER_SLUG);
  ...
  const hasProAccess = () => {
    const hasPolarSub = STARTER_TIER ? isCurrentPlan(STARTER_TIER) : false;
  ...
  const getProAccessSource = () => {
    if (STARTER_TIER && isCurrentPlan(STARTER_TIER)) return 'polar';
              ...
              {hasProAccess() ? (
                ...
                !user ? (
                  <Button 
                    ...
                    onClick={() => STARTER_TIER && STARTER_SLUG && handleCheckout(STARTER_TIER, STARTER_SLUG)}
                  ...
                  <div className="space-y-3">
                    <Button 
                      className="w-full group" 
                      onClick={() => STARTER_TIER && STARTER_SLUG && handleCheckout(STARTER_TIER, STARTER_SLUG, 'dodo')}
                      disabled={!paymentsEnabled}
                    >
                    ...
                    <Button
                      variant="outline"
                      className="w-full group"
                      onClick={() => STARTER_TIER && STARTER_SLUG && handleCheckout(STARTER_TIER, STARTER_SLUG, 'polar')}
                      disabled={!paymentsEnabled}
                    >
                ...
                <Button
                  className="w-full group"
                  onClick={() => STARTER_TIER && STARTER_SLUG && handleCheckout(STARTER_TIER, STARTER_SLUG)}
                  disabled={location.loading || !paymentsEnabled}
                >
